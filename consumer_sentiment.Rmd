...
# add u3 delta
test_df$u3_del <- c(NA, diff(test_df$u3))

precovid <- df[df$DATE <= "2020-02-01",]
precovidtest <- test_df[test_df$DATE <= yearmonth("2020-02-01"),]
...
model2 <- lm(curr_index ~ u3 + cpi_total + cpi_yoy + cpi_yoy_del, data = precovid)
model3 <- lm(curr_index ~ u3 + u3_del + cpi_yoy + cpi_yoy_del + fed_funds,
             data=precovidtest)
```
...
predicted3 <- data.frame(pred = predict(model3, precovidtest), DATE = precovid$DATE)
...
predicted3 <- data.frame(pred = predict(model3, test_df), DATE = df$DATE)
